...
def main(args=None):
   ...
   # check if microbiome_path file extensions are valid
    ...
    # microbiomes_assemblies["assembly_id"] = microbiomes_assemblies["assembly_id"].apply(lambda x: 'a' + str(x))
    ...
    microbiomes_assemblies[["assembly_id", "assembly_path"]].drop_duplicates().to_csv(args.assemblies, sep="\t", index=False)
